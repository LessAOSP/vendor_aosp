...
function githubremote()
{
    ...
    local REMOTE=$(git config --get remote.lessaosp.projectname)
...
function pixelgerrit() {
    ...
    local user=`git config --get review.gerrit.lessaosp.org.username`
    local review=`git config --get remote.lessaosp.review`
    local project=`git config --get remote.lessaosp.projectname`
...
function pixelrebase() {
    ...
    cd $dir
    repo=$(git config --get remote.lessaosp.projectname)
    echo "Starting branch..."
    repo start tmprebase .
    echo "Bringing it up to date..."
    repo sync .
    echo "Fetching change..."
    git fetch "http://gerrit.lessaosp.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
